(() => {
    ...
    (() => {
        ...
        document.addEventListener("click", (function(e) {
            ...
            if (targetElement.closest(".checkbox") && targetElement.closest(".filter__block") && targetElement.tagName !== "INPUT") filterClearShow(targetElement);
            if (targetElement.closest(".filter__clear")) {
                const clearButton = targetElement.closest(".filter__clear");
                filterBlockClear(clearButton);
            }
            if (targetElement.closest(".filter__title")) {
                const buttonTitle = targetElement.closest(".filter__title");
                filterBlockOpen(buttonTitle);
            }
        }));
        ...
        function filterBlockOpen(buttonTitle) {
            const filterBlock = buttonTitle.parentElement.closest(".filter__block");
            filterBlock.classList.toggle("_filter-open");
        }
        let filterColorItems = document.querySelectorAll(".color-filter__item .checkbox__label");
        filterColorItems.forEach((item => {
            let colorLabel = item.querySelector(".checkbox__text").textContent;
            item.style.setProperty("--background-color", colorLabel);
            if (colorLabel == "White") item.classList.add("_white");
        }));
        let currentPath = window.location.pathname;
        console.log(currentPath);
        let navigationalElements = document.querySelectorAll("[data-target]");
        navigationalElements.forEach((function(element) {
            let targetPath = element.getAttribute("data-target");
            if (targetPath === currentPath) element.classList.add("_active"); else element.classList.remove("_active");
        }));
        window["FLS"] = true;
        isWebp();
        menuInit();
